...
public:
	...
	class listLenComparator {
	public:
		inline bool operator()(lptr* a, lptr* b) { return a->lengthOfList<b->lengthOfList;} //to sort by list length -- aka comparelps
	};
...
public:
	...
	inline void sortByListLen() {
		std::sort(begin(),end(),lptr::listLenComparator()); //TODO: speedup
		for(unsigned int i=0; i<size(); ++i)
			this->operator[](i)->indexInSorted = i;
	}
...
public:
	...
	void fill_cache(const std::string sql_path,
