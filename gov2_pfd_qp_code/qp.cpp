...
class resultsDump {
	FILE *fresult_log;
public:
	resultsDump(const char* result_) {
...
/* Query Processing */
void QueryProcessing::operator()(const char* queryLog, const int topk,
	const std::string result_path, const std::string doclength_file,
	const std::string basic_table){
	resultsDump resLogger(result_path.c_str());
	profilerC& p = profilerC::getInstance();

	loadDocLen(doclength_file.c_str());
	cout << "load doc lengths done\n";
	// load terms -> termID into map for Standard Index
	termsMap lex;
	fillTermsMap(lex, basic_table);
	// Standard Index
	...
	while( qn++ < 100) {
		...
		wand.GoodAnd(lps, topk, res, seek);
		// wand(lps, topk, res);
		total_seek += seek;
