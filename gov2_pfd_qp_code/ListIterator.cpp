...
std::vector<lptr> load(const std::string sql_path,
	const std::string term_path){
	SqlProxy ld(sql_path);
	sqlite3_stmt* ppStmt = ld.prepare("select * from terms"); //tODO: make iterator
	if(! ppStmt)
  ...
  while (sqlite3_step(ppStmt) == SQLITE_ROW) {  // For each row returned
	  ...
	  c.cpool.setName(term_path + "/" + c.term);
...
void termsCache::fill_cache(const std::string sql_path,
	const std::string term_path){
	onDemandCpool::initPool(CONSTS::STORAGE_CAPACITY);
	data = load(sql_path, term_path);
